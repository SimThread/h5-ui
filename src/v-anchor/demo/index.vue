<template>
  <div class="demo-wrap">
    <!-- 導航 -->
    <nav
      class="block detail-nav"
      ref="navItem"
      style="margin-top: 10px; position: relative; z-index: 100"
    >
      <div
        class="nav-list"
        :class="{ 'fexd-nav': isFixed }"
        :style="{ top: `0px` }"
      >
        <span class="nav-item active" v-anchor="1" anchor-distance="40"
          >屋苑資料</span
        >
        ...
        <span class="nav-item" v-anchor="5" anchor-distance="40"
          >地圖及周邊</span
        >
      </div>
    </nav>
    ...
    <!-- 樓盤信息 -->
    <div class="page-block page-block1" id="anchor-1" ref="pageBlock">
      屋苑资料
    </div>

    <!-- 成交記錄 -->
    <div class="page-block page-block2" id="anchor-2" ref="pageBlock">
      成交記錄
    </div>

    <!-- 相關學校 -->
    <div class="page-block page-block3" id="anchor-3" ref="pageBlock">
      相關放盤
    </div>

    <!-- 地圖及周邊 -->
    <div class="page-block page-block4" id="anchor-4" ref="pageBlock">
      相關學校
    </div>

    <!-- 相關放盤 -->
    <div class="page-block page-block5" id="anchor-5" ref="pageBlock">
      地圖及周邊
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    onScroll() {
      const headerEl = document.querySelector('.demo-nav__title');
      const navEl = document.querySelector('.nav-list');

      // return;
      const headHeight = headerEl.clientHeight;
      ...
      document.querySelectorAll('.page-block').forEach((item, index) => {
        ...
        if (isScrollToBottom()) {
          ...
          document
            ...
            [document.querySelectorAll('.page-block').length - 1].classList.add(
              'active'
            );
          return;
        }
