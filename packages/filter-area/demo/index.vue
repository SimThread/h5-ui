...
areaDistrict[0] = [];
for (let key in districtId) {
  areaDistrict[0].push(Number(key));
}

for (let key in areaId) {
...
for (let key in subwayStationData) {
...
export default {
    ...
    this.$nextTick(() => {
      ...
      if (queryPrams.areaId) {
        ...
        if (queryPrams.districtId) {
          ...
          const districtDefaultIndex = districtId.map((id) => {
            return subOptions.areaOptions[queryPrams.areaId].findIndex((option) => Number(id) === option.id);
          });
        ...
        if (queryPrams.subwayStation) {
          ...
          const subwayStationDefaultIndex = subwayStation.map((id) => {
            return subOptions.subwayOptions[queryPrams.subwayLine].findIndex((option) => Number(id) === option.id);
          });
      ...
      if (queryPrams.ageOfBuildingSearch) {
        let index = ageOfBuilding.findIndex((item) => item.value == queryPrams.ageOfBuildingSearch);
        this.age.select.setColumnIndex(0, index);
      }

      // 設置排序
      if (queryPrams.sort) {
        let index = sortOptions.findIndex((item) => item.id == queryPrams.sort);
  ...
  methods: {
    ...
    onPriceConfirm(minPrice, maxPrice) {
      this.price.select.setColumnIndex(0, -1);
      minPrice = parseInt(minPrice.trim()) || 0;
      maxPrice = parseInt(maxPrice.trim()) || 200000000;
