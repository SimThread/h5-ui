...
在图形化界面中，点击`依赖` -> `安装依赖`，然后将 `h5-ui` 添加到依赖中即可。
...
> 注意：在配置 postcss-loader 时，应避免 ignore node_modules 目录，这会导致 h5-ui 的样式无法被编译
